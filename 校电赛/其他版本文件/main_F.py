...
task3 = [1, 8, 1, 7, 1, 8, 1, 6, 1, 5, 1, 6, 5, 1, 8, 1, 7, 1, 9, 10, 0]
task4 = [1, 3, 1, 7, 1, 8, 1, 6, 5, 1, 9-L2R_flag, 1, 7, 1, 9, 1, 10, 0]
# 距离判断数据
dis_min = 15 # 走进到方块范围内
#dis_mid = 90 # 走到赛道中间
#dis_max = 70 # 走完赛道全段
...
def Detect_4(mode_pos, Done_flag):
    ...
    #if mode_pos == 10 #状态10左右转判断
        #mode = 9 - L2R_flag

#    状态转变
    if change:
        if mode_pos == 5 # 测试判断随机障碍物
            if not (distance > 90 and distance < 120):
                car.turn_right(190)
                L2R_flag = 1
            elif
                L2R_flag = 0
...
while True:
    ...
    mode_pos, Done_flag = Detect_3(mode_pos, Done_flag)  # 判断当前模式的触发条件是否满足
    if mode_pos < len(task3):
        Done_flag = Run(task3[mode_pos]) # 执行当前模式的操作
        print(task2[mode_pos])
    # print(distance)
#while True:
    #LCM_Calculate()
    #print(distance)
